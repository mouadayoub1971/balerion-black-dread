...
- [x] Create images S3 bucket
- [x] Create frontend S3 bucket
- [x] Configure bucket policies (public read for frontend, private for images with signed URLs)
- [x] Enable CORS on images bucket
...
- [x] Create ChatMessages table
- [x] Add GSI if needed (e.g., for user queries)
- [x] Verify table creation: `aws dynamodb describe-table --table-name ChatMessages`
...
- [x] Store Gemini API key
- [x] Note parameter name for Lambda environment variables: `/chatbot/gemini-api-key`
...
exports.handler = async (event) => {
  try {
    ...
    const functionCall =
      geminiResponse.candidates[0]?.content?.parts[0]?.functionCall;
